...
void main() {
  const mnemonics =
      "describe beyond repair shuffle pluck during still prefer gravity film green master";

  test('test connecting to sui network', () async {
    /// connect to devnet
    final devnetClient = SuiClient(SuiUrls.devnetAPI);

    /// connect to  testnet
    final testnetClient = SuiClient(SuiUrls.testnetAPI);

    /// connect to  mainnet
    final mainnetClient = SuiClient(SuiUrls.mainnetAPI);
  });
  ...
  test('test querying events', () async {
    final client = SuiClient(SuiUrls.mainnetAPI);
  ...
  test('test ed25519 transfer sui', () async {
    ...
    final account =
        SuiAccount.fromMnemonics(mnemonics, SignatureScheme.Ed25519);
    final client = SuiClient(SuiUrls.devnetAPI, account: account);
    ...
    final txn =
        PaySuiTransaction(inputObjectIds, [recipient], [1000], gasBudget);
  ...
  test('test programmable transaction blocks', () async {
    ...
    final waitForLocalExecutionTx =
        await client.signAndExecuteTransactionBlock(account, tx);
    debugPrint(waitForLocalExecutionTx.digest);
  });
}
