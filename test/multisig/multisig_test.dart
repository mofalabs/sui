...
void main() {
  group('Multisig scenarios', () {
    test('multisig address creation and combine sigs using Secp256r1Keypair',
        () async {
      ...
      expect(k3.getPublicKey().verifyTransaction(bytes, signature), true);
      ...
      expect(publicKey2.verifyTransaction(bytes, multisig), true);
    });
    ...
    test(
      ...
      final isValidSig1 =
          k1.getPublicKey().verifyPersonalMessage(signData, sig1.signature);
      final isValidSig2 =
          k2.getPublicKey().verifyPersonalMessage(signData, sig2.signature);
      ...
      expect(
          () => multiSigPublicKey
              .combinePartialSignatures([sig1.signature, sig3.signature]),
          throwsArgumentError);

      final parsed = parseSerializedSignature(multisig);
      final publicKey = MultiSigPublicKey(parsed.multisig!.multisigPK);

      expect(() => publicKey.verifyPersonalMessage(signData, multisig),
          throwsArgumentError);
    });
    ...
    test('providing invalid signature', () {
      ...
      expect(() => multiSigPublicKey.combinePartialSignatures([sig3.signature]),
    ...
    test('providing signatures with invalid order', () {
      ...
      final multisig = multiSigPublicKey
          .combinePartialSignatures([sig2.signature, sig1.signature]);
      ...
      expect(() => publicKey.verifyPersonalMessage(signData, multisig),
          throwsException);
      expect(() => multiSigPublicKey.verifyPersonalMessage(signData, multisig),
          throwsException);
    });
    ...
    test('providing invalid intent scope', () {
      ...
      // Invalid intentScope.
      expect(() => publicKey.verifyTransaction(signData, multisig),
          throwsException);
      expect(() => multiSigPublicKey.verifyTransaction(signData, multisig),
          throwsException);
    });
    ...
    test('providing empty values', () {
      ...
      expect(publicKey.verifyTransaction(signData, multisig), false);
      expect(multiSigPublicKey.verifyTransaction(signData, multisig), false);
    });
  });
  ...
  group('Multisig address creation:', () {
    ...
    test('`combinePartialSigs()` with zklogin sigs', () {
      ...
      final multisig =
          publicKey.combinePartialSignatures([sig1.signature, zklogin_sig]);
      expect(
        multisig,
