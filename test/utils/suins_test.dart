...
void main() {
  ...
  group('normalizeSuiNSName', () {
    ...
    test('normalize SuiNS names', () {
      ...
      expect(
          normalizeSuiNSName('1.example.sui', SuiNSType.dot), '1.example.sui');
      expect(normalizeSuiNSName('1@example', SuiNSType.dot), '1.example.sui');
      expect(normalizeSuiNSName('a.b.c.example.sui', SuiNSType.dot),
          'a.b.c.example.sui');
      expect(normalizeSuiNSName('A.B.c.123@Example', SuiNSType.dot),
      ...
      expect(() => normalizeSuiNSName('-@test'),
          throwError('Invalid SuiNS name -@test'));
      expect(() => normalizeSuiNSName('-1@test'),
          throwError('Invalid SuiNS name -1@test'));
      expect(() => normalizeSuiNSName('test@-'),
          throwError('Invalid SuiNS name test@-'));
      expect(() => normalizeSuiNSName('test@-1'),
          throwError('Invalid SuiNS name test@-1'));
      expect(() => normalizeSuiNSName('test@-a'),
          throwError('Invalid SuiNS name test@-a'));
      expect(() => normalizeSuiNSName('test.sui2'),
          throwError('Invalid SuiNS name test.sui2'));
      expect(() => normalizeSuiNSName('.sui2'),
          throwError('Invalid SuiNS name .sui2'));
      expect(() => normalizeSuiNSName('test@'),
          throwError('Invalid SuiNS name test@'));
      expect(
          () => normalizeSuiNSName('@@'), throwError('Invalid SuiNS name @@'));
      expect(() => normalizeSuiNSName('@@test'),
          throwError('Invalid SuiNS name @@test'));
      expect(
      ...
      expect(() => normalizeSuiNSName('@test.test'),
          throwError('Invalid SuiNS name @test.test'));
      expect(() => normalizeSuiNSName('#@test'),
          throwError('Invalid SuiNS name #@test'));
      expect(() => normalizeSuiNSName('test@#'),
          throwError('Invalid SuiNS name test@#'));
      expect(() => normalizeSuiNSName('test.#.sui'),
          throwError('Invalid SuiNS name test.#.sui'));
      expect(() => normalizeSuiNSName('#.sui'),
          throwError('Invalid SuiNS name #.sui'));
      expect(() => normalizeSuiNSName('@.test.sue'),
