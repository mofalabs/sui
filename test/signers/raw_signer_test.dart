...
void main() {
  ...
  setUp(() {
    ...
    ed25519Keypair =
        Ed25519Keypair.fromSecretKey(base64Decode(VALID_Ed25519_SECRET_KEY));
  });
  ...
  test('transfer sui with secp256r1 keypair', () async {
    final signer = RawSigner(secp256r1Keypair, endpoint: SuiUrls.devnetAPI);
    final coins = await signer.getCoins(signer.getAddress());
    final txn = TransferSuiTransaction(
        coins.data[0].coinObjectId, DEFAULT_GAS_BUDGET, DEFAULT_RECIPIENT, 100);
    final resp = await signer.transferSui(txn);
  ...
  test('pay with secp256k1 keypair', () async {
    ...
    final txn = PayTransaction(inputObjectIds, [DEFAULT_RECIPIENT], [1000],
        DEFAULT_GAS_BUDGET, coins.data[2].coinObjectId);
  ...
  test('pay all sui with secp256k1 keypair', () async {
    ...
    final txn = PayAllSuiTransaction(
        inputObjectIds, DEFAULT_RECIPIENT_2, DEFAULT_GAS_BUDGET);
  ...
  test('test getGasCostEstimation', () async {
    ...
    var txn = PaySuiTransaction(
        inputObjectIds, [DEFAULT_RECIPIENT], [1000], DEFAULT_GAS_BUDGET);
