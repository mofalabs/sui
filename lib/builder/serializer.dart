...
BcsType? getPureBcsSchema(dynamic typeSignature) {
  ...
  if (typeSignature is Map<String, dynamic> &&
      typeSignature.containsKey('vector')) {
    if (typeSignature['vector'] == 'u8') {
      return SuiBcs.VECTOR(SuiBcs.U8).transform(
        input: (dynamic val) =>
            val is String ? Uint8List.fromList(val.codeUnits) : val,
        output: (val) => val,
      );
    }
  ...
  if (typeSignature is Map<String, dynamic> &&
      typeSignature.containsKey('datatype')) {
  ...
  if (type['Struct'] != null) {
    return {
      "datatype": {
        ...
        "typeParameters": type["Struct"]["typeArguments"]
            .map(normalizedTypeToMoveTypeSignatureBody),
      },
    };
  }
